...
modelPath = os.environ["PathToModel"]
inputDataFolder = os.environ["PathToData"]
...
# get data
data = get_data("test", IMAGE_SHAPE,MULTI_PREDICTION = True, image_format=IMAGE_TYPE, SAVE_CSV=False, min=MIN, max=MAX, Path_In=inputDataFolder, Path_Out = outputFolder)
...
SAVE_CSV = False
if SAVE_CSV:
    toSave_CVS = outputFolder+'/DualNet-ENE_testresults.csv'
    if not toSave_CVS:
    	os.makedirs(toSave_CVS)
    results.to_csv(toSave_CVS) # Can change filename as needed
