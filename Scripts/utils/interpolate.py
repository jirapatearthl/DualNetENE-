...
def sitk_interpolation(path_to_nrrd, interpolation_type, new_spacing):
    ...
    resampleImageFilter.SetSize(new_size)
    resampleImageFilter.SetTransform(sitk.Transform())
    #resampleImageFilter.SetSize(new_size)
    resampleImageFilter.SetInterpolator(interpolation_type)
    resampleImageFilter.SetOutputOrigin(data.GetOrigin())
    resampleImageFilter.SetDefaultPixelValue(0)
    
    resampleImageFilter.SetOutputSpacing([float(x) for x in new_spacing])
